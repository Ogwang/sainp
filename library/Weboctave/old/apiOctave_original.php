...
Class Finance_apiOctave extends Finance_Matrix {
	...
	private function _createFile($Alb = null) {
		...
		$f = fopen($this->_workOutputFile, "w");
		fwrite($f, $script);
		fclose($f);
	}
	
	private function _callOctave() {
		//ob_start();
		//exec("C:\\Octave\\3.2.0_gcc-4.3.0\\bin\\octave.exe -q $this->_workOutputFile", $octaveResult);
		
		exec("C:\\Octave\\Octave\\bin\\octave.exe -q $this->_workOutputFile", $octaveResult);
		//$octaveResult = ob_get_contents();
		list($X,$Obj) = $this->_optResult($octaveResult); 
		//ob_end_clean();
		unlink($this->_workOutputFile);
		return array($X,$Obj);
	}
